EXPERIENCE_PATTERNS = [
    ...
    r"(?:experiencia|experience).*?(\d+)\s*(?:-|a|to|y)\s*(\d+)\s*(?:años|years)",
    r"(\d+)\s*(?:-|a|to|y)\s*(\d+)\s*(?:años|years).*?(?:experiencia|experience)",
    # Número + años de experiencia: "5 años de experiencia"
    r"(\d+)\+?\s*(?:años|years)\s*(?:de|of)\s*(?:experiencia|experience)",
